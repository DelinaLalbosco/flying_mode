...
# Note: drdds is already installed on the robot under /opt/ros/foxy, so we only copy S10_sdk_deploy.
ssh user@10.21.31.103 'mkdir -p ~/S10_sdk_deploy/src'
scp -r ~/sdk_deploy/src/S10_sdk_deploy user@10.21.31.103:~/S10_sdk_deploy/src

# ssh connect for remote development, 
ssh user@10.21.31.103
cd S10_sdk_deploy
source /opt/ros/foxy/setup.bash #source ROS2 env
# Build only s10_sdk_deploy. Do NOT rebuild drdds on the robot —
