...
@brief simulation in mujoco with compliant adaptive navigation & stuck recovery
...
# Rule-Compliant Update: Default reach radius increased from 0.2m to 0.4m
TRACK_REACH_RADIUS = float(os.environ.get("S10_TRACK_REACH_RADIUS", "0.4"))
...
class MuJoCoSimulationNode(Node):
    def __init__(self,
        ...
        # Stuck Recovery Mechanism State
    ...
    def _auto_nav_step(self):
        ...
        # Check for Stuck condition (Rule compliant terrain check)
        if self.last_check_pos is None:
            ...
            if disp < 0.1 and not self.recovery_mode:
                ...
                self.get_logger().warn(f"[NAV-RECOVERY] Stuck detected! Displacement={disp:.3f}m. Triggering recovery move.")
                self.recovery_mode = True
                self.recovery_timer = self.timestamp
            self.last_check_pos = robot_pos.copy()
            self.last_check_time = self.timestamp

        # Handle Recovery Maneuver
        if self.recovery_mode:
            if self.timestamp - self.recovery_timer < 1.5:  # Reverse & turn away for 1.5s
                twist = Twist()
                twist.linear.x = -0.2
                twist.angular.z = 0.5
        ...
        # Adaptive Speed Scaling based on orientation alignment
        turn = float(np.clip(yaw_error * 1.2, -0.6, 0.6))
        if abs(yaw_error) > 0.8:
            forward = 0.0  # Turn in place when facing away
        else:
            forward = float(0.5 * np.cos(yaw_error))
