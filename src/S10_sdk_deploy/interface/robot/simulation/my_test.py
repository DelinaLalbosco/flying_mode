...
class MuJoCoSimulationNode(Node):
    ...
    def _navigation_controller(self):
        ...
        # print(f"Current robot position: {robot_pos}")
        # print(f"Current robot position target: {target}")
        # print(f"Current robot position distance: {distance}")
        print(f"Current robot position spped forward: {forward_cmd} Andle {turn_cmd}")
   
        direction = self.get_direction_label(heading_error, forward_cmd)
        self.get_logger().info(f"[NAVIGATION] Direction: {direction}")
        import time
        time.sleep(0.1)
    ...
    def get_direction_label(self, heading_error, forward_cmd, angle_thresh=None):
        """
        Convert current navigation decision into a simple direction label.
        Call this from inside _navigation_controller() where heading_error
        and forward_cmd are already computed.
        """
        if angle_thresh is None:
            angle_thresh = HEADING_SLOWDOWN_ANGLE

        if forward_cmd == 0.0 and abs(heading_error) >= HEADING_STOP_ANGLE:
            return "STOP_TURN"

        if abs(heading_error) > angle_thresh:
            return "L" if heading_error > 0 else "R"

        return "F"  # your controller only ever drives forward, never backward/sideways
    ...
    def _send_navigation_command(
        ...
        # print(f"Current robot position spped x: {self.commanded_base_x} y {self.commanded_base_y} z {self.commanded_base_z} half_yaw {new_yaw}")
        # import time
        # time.sleep(0.1)
