...
class MuJoCoSimulationNode(Node):
    def __init__(self,
        ...
        # These valuse are configured to set the robot position
    ...
    def _init_track_progress(self):
        ...
        self.get_logger().info(f"Points related : {point_related_geoms}")
        ...
        self.get_logger().info(
            ...
            f"waypoints."
            f"{self.track_waypoint_positions}"
        )
        ...
        if self.track_body_id < 0:
            ...
            return
        self.get_logger().warn(f"robot body pos: {self.track_body_id}")
